...
export default function Cart() {
  ...
  const handleDiscount = async (totalPrice) => {
    ...
    const addPoints = await fetch("http://localhost:3000/api/v1/user/addPoints", {
      method: 'POST',
      headers: {
  ...
  return (
    <div>
      <div className='container m-auto mt-5 table-responsive table-responsive-sm table-responsive-md'>
          ...
          <tbody>
            {data.map((food, index) => (
              <tr key={index}>
                ...
                <td>{food.shopName}</td>
                <td>
                  <MdDelete onClick={() => { dispatch({ type: "REMOVE", index: index }) }}>Delete</MdDelete>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div><h1 className='fs-2'>Discount: {discount}/-</h1></div>
        <div><h1 className='fs-2'>Total Price after Discount: {totalPrice - discount}/-</h1></div>
        <div>
          <button className='btn bg-success mt-5' onClick={() => handleDiscount(totalPrice)}>Check Discount</button>
        </div>
        <div>
